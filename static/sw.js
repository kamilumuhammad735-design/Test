...
const CACHE_NAME = 'socialpay-v9';
const DATA_CACHE = 'socialpay-data-v9';
...
const CACHEABLE_PAGES = [
  '/dashboard','/tasks','/balance','/profile',
...
self.addEventListener('push', event => {
  ...
  try {
    ...
    event.waitUntil(self.registration.showNotification(data.title || 'SocialPay', {
      body: data.body || '', icon: '/static/icons/icon-192.svg',
      badge: '/static/icons/icon-192.svg', data: { url: data.url || '/dashboard' }, vibrate: [200,100,200],
    }));
  } catch {}
});
...
self.addEventListener('notificationclick', event => {
  ...
  event.waitUntil(
    self.clients.matchAll({ type: 'window' }).then(clients => {
      const e = clients.find(c => c.url.includes(url));
      return e ? e.focus() : self.clients.openWindow(url);
    })
  );
});
